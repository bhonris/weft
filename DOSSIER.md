...
## Current status
Phase: **Cycle 7 — Time Leap Development** (Leap 51/62, expansion cycle 7). Divergence meter: 98.56% statements (350 tests). Operator opened Cycle 7 — **remappable keybindings + dispatch unification** (spec `## Expansion 7`). Shipped so far: (leap 49) KeyAction/CommandId **dual dispatch unified** — chords resolve to a `CommandId` through the single `runCommand`, bidirectional no-drift test (structural fix for the Cycle-6 `viewer.save` drift); (leap 50) **`routeKey` data-driven** over `DEFAULT_KEYMAP` via `chordOf`, passthrough tests preserved; (leap 51) **protected-chord guard + rebind/reset API** — pure `isProtectedChord`/`bindChord`/`resetChord`/`resetAll` deliver criteria 3 (protected unbindable) & 4 (conflict detection). Next: persist user overrides (WorkspaceState v3→v4) and wire the merged keymap into routeKey; then the keybindings editor UI. Two manual pre-cycle enhancements also landed: the `PostToolUse` waiting→working fix and a notifications on/off toggle (workspace schema v3).
