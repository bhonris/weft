...
## Current status
Phase: **Cycle 7 — Time Leap Development** (Leap 50/62, expansion cycle 7). Divergence meter: 98.55% statements (350 tests). Operator opened Cycle 7 — **remappable keybindings + dispatch unification** (spec `## Expansion 7`). Shipped so far: (leap 49) the KeyAction/CommandId **dual dispatch is unified** — chords resolve to a `CommandId` via pure `core/commands/action-dispatch.ts` through the single `runCommand`, with a bidirectional no-drift test (the structural fix for the Cycle-6 `viewer.save` drift); (leap 50) **`routeKey` is data-driven** — it canonicalizes an event via `chordOf` and resolves against `DEFAULT_KEYMAP` (`core/keybindings/keymap.ts`), the prerequisite for user remapping, with all existing passthrough tests preserved. Next: a persisted user keymap with a protected-set guard + reset-to-defaults. Two manual pre-cycle enhancements also landed: the `PostToolUse` waiting→working fix and a notifications on/off toggle (workspace schema v3).
