...
## Current status
Phase: **Cycle 7 — Time Leap Development** (Leap 52/62, expansion cycle 7). Divergence meter: 98.61% statements (375 tests). Operator opened Cycle 7 — **remappable keybindings + dispatch unification** (spec `## Expansion 7`). Shipped so far: (49) dual dispatch unified + no-drift test; (50) `routeKey` data-driven over `DEFAULT_KEYMAP` via `chordOf`; (51) protected-chord guard + rebind/reset API — criteria 3 & 4; (52) **persistence** — `WorkspaceState.keymapOverrides` (chord→command id) schema v3→v4, `actionForCommand` + `buildKeymap` merge, App feeds the effective keymap to `routeKey` via a live ref, `general.resetKeybindings`, all persisted. Next: the keybindings **editor UI** (criterion 5) — checking it off also completes criterion 2 (user-editable + persisted). Then the deferred palette/explorer fixes (crit 6–8). Queued after Cycle 7: **Cycle 8 = in-project file tabs + always-present moveable CLI dock** (brief: `documents/split-pane-workspace.md`). Two manual pre-cycle enhancements also landed: the `PostToolUse` waiting→working fix and a notifications on/off toggle (workspace schema v3).
