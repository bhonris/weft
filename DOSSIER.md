...
## Current status
Phase: **Cycle 7 — Time Leap Development** (Leap 49/62, expansion cycle 7). Divergence meter: 98.54% statements (350 tests). Operator opened Cycle 7 — **remappable keybindings + dispatch unification** (spec `## Expansion 7`). First leap shipped: the KeyAction/CommandId **dual dispatch is unified** — chords resolve to a `CommandId` via pure `core/commands/action-dispatch.ts` and run through the single `runCommand`, with a bidirectional no-drift test (the structural fix for the Cycle-6 `viewer.save` drift). Next: a persisted user-editable keymap with reset-to-defaults. Two manual pre-cycle enhancements also landed: the `PostToolUse` waiting→working fix and a notifications on/off toggle (workspace schema v3).
